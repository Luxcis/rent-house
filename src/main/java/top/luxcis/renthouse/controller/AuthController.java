...
public class AuthController {
    ...
    @SaIgnore
    @PostMapping("doLogin/dev")
    @ApiLog(type = LogType.LOGIN)
    @Operation(summary = "测试环境登陆", description = "测试环境登陆", method = "POST")
    @ApiResponse(responseCode = "200", description = "成功", content = @Content(schema = @Schema(implementation = Resp.class)))
    public Resp<String> devLogin() {
        if (SpringUtil.getActiveProfile().startsWith("dev-")) {
            log.warn("测试环境一键登录");
            StpUtil.login("365309554988793856");
            return Resp.SUCCESS(StpUtil.getTokenValue());
        } else {
            return Resp.ERROR(RespEnum.NOT_FOUND, null);
        }
    }
    ...
    @ApiResponse(responseCode = "200", description = "成功", content = @Content(schema = @Schema(implementation = Resp.class)))
    public Resp<String> doLogin(@NotBlank String code) {
        return Resp.SUCCESS(userService.loginByWechatMPCode(code));
    }
    ...
    public Resp<User> userInfo() {
        return Resp.SUCCESS(userService.getMapper().selectOneWithRelationsById(StpUtil.getLoginIdAsString()));
    }
